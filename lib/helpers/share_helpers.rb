...
module ShareHelpers
  ...
  def mail_share_url(article)
    ...
    body = ERB::Util.url_encode(full_url(article.path, { source: 'Email' }))
  ...
  def sms_share_url(article)
    ...
    url = full_url(article.path, { source: 'Message' })
    text = "#{title} #{url}"
  ...
  def facebook_share_url(article)
    url = ERB::Util.url_encode(full_url(article.path, { source: 'Facebook' }))
  ...
  def reddit_share_url(article)
    ...
    url = ERB::Util.url_encode(full_url(article.path, { source: 'Reddit' }))
  ...
  def threads_share_url(article)
    title = sanitize(article.title)
    url = full_url(article.path, { source: 'Threads' })
    text = "#{title} #{url}"
  ...
  def bluesky_share_url(article)
    title = sanitize(article.title)
    url = full_url(article.path, { source: 'Bluesky' })
    text = "#{title} #{url}"
    encoded_text = ERB::Util.url_encode(text)
