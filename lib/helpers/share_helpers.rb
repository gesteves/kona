...
module ShareHelpers
  ...
  def mail_share_url(article)
    subject = ERB::Util.url_encode(sanitize(article.title, ignore_html_entities: true))
  ...
  def sms_share_url(article)
    title = sanitize(article.title, ignore_html_entities: true)
  ...
  def reddit_share_url(article)
    title = ERB::Util.url_encode(sanitize(article.title, ignore_html_entities: true))
