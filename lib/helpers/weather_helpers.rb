...
module WeatherHelpers
  ...
  def sunrise
    Time.parse(todays_forecast.sunrise).in_time_zone(data.location.time_zone.timeZoneId)
  end
  ...
  def tomorrows_sunrise
    Time.parse(tomorrows_forecast.sunrise).in_time_zone(data.location.time_zone.timeZoneId)
  end

  # Retrieves the time of sunset for today.
  # @return [Time, nil] The time of sunset today, or nil if not found.
  def sunset
    Time.parse(todays_forecast.sunset).in_time_zone(data.location.time_zone.timeZoneId)
  end
