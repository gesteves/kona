...
module ArticleHelpers
  ...
  def trending_articles(count: 4, exclude: nil)
    data.articles
      ...
      .reject { |a| a.draft }                 # Exclude drafts
      .reject { |a| a.entry_type == 'Short' } # Exclude short posts
      .sort_by { |a| [-absolute_trending_score(a), -article.metrics[:"1d"].pageviews, -article.metrics[:"7d"].pageviews, -article.metrics[:"30d"].pageviews, -article.metrics.all.pageviews] } # Sort by trending score, then use pageviews as tiebreakers
      .take(count)
  end
