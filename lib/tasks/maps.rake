...
namespace :maps do
  ...
  task generate: :environment do
    Dir.glob(File.join(StaticMap::GPX_FOLDER, '*.gpx')).each do |gpx_file|
      map = StaticMap.new(gpx_file, { reverse_markers: ENV['REVERSE_MARKERS'].present?, padding: ENV['PADDING'], max_height: ENV['MAX_HEIGHT'], min_size: ENV['MIN_SIZE'], tileset_id: ENV['TILESET_ID'] }.compact)
