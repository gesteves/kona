...
# The `rspec --init` command made this file. Usually, each spec is in a `spec`
# directory, which RSpec adds to the `$LOAD_PATH`. The `.rspec` file contains
# `--require spec_helper`, which loads this file always. Thus no other file
# needs to require it.
#
...
RSpec.configure do |config|
  ...
  # Run the specs in a random order, to find a dependency on the order. To make
  # a failure occur again, give the seed from the last run: --seed 1234
  config.order = :random

  # The `--seed` option gives the seed for the random numbers in this process.
  # Thus the random values in an example also occur again.
...
=begin
  ...
  # This setting makes the warnings appear. Use it, but a dependency with
  # problems can give too many warnings.
