...
RSpec.describe ContactMailJob do
  ...
  it "emails a clean submission to the owner with Reply-To set to the sender" do
    allow(akismet).to receive(:spam?).and_return(false)

    described_class.new.perform("Jane Rider", "jane@example.com", "Hello!", "203.0.113.7", "Mozilla/5.0")
    ...
    expect(mailer).to have_received(:send_email).with(
      hash_including(to: "owner@example.test", reply_to: "jane@example.com",
