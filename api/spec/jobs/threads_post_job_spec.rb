...
RSpec.describe ThreadsPostJob do
  ...
  describe "a thread" do
    ...
    # ⚠️ It adds the next post AT ONCE. A delay went in here for a 500 on a reply container, on the
    # reasoning that Meta was not ready for the parent. That was wrong: the delay changed nothing,
    # and the token in the query string was the difference. Do not add a wait here again with no
    # evidence for it.
    it "adds the next post with no delay" do
      described_class.new.perform(thread)
