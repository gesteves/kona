...
class ArticleTaxonomy
  ...
  def initialize(articles:, tree: {})
    @tree = tree || {}
    @weights = articles.each_with_object({}) { |article, acc| acc[article.sys&.id] = expand(article) }
  ...
  def overlap(a_id, b_id)
    ...
    (a.keys | b.keys).each do |concept|
      weight = @idf[concept] || 0.0
      next if weight.zero?

      a_weight = a[concept] || 0.0
  ...
  def race_concept_id(article)
    Array(article.concept_ids)
      ...
      .map { |id| [ id, TaxonomyConcepts.ancestor_ids(id, @tree).size + 1 ] }
  ...
  def expand(article)
    ...
    direct.each do |id|
      TaxonomyConcepts.ancestor_ids(id, @tree).each do |ancestor|
